...
class br6d_Dataset:
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg.get(
            "dataset_root",
            osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/train"),
        )
...
br6d_model_root = "/media/gouda/3C448DDD448D99F2/segmentation/gdrnpp_bop2022/datasets/BOP_DATASETS/br6d/models"
################################################################################

br6d_BOP19_20_OBJS = [ref.br6d.id2obj[_i] for _i in [3]]

SPLITS_BR6D_PBR = dict(
    br6d_pbr_train=dict(
        name="br6d_pbr_train",
        objs=ref.br6d.objects,  # all 33 objects
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/train"),
    ...
    # br6d_bop_train_pbr=dict(
    #    name="br6d_bop_train_pbr",
    #    objs=br6d_BOP19_20_OBJS,  # selected 16 objects for BOP19/20
    #    dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/train_pbr"),
    #    models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/models"),
    #    scale_to_meter=0.001,
    #    with_masks=True,  # (load masks but may not use it)
    #    with_depth=True,  # (load depth path here, but may not use it)
...
for obj in ref.br6d.objects:
    for split in ["train"]:
        name = "br6d_{}".format(obj, split)
        if name not in SPLITS_BR6D_PBR:
            SPLITS_BR6D_PBR[name] = dict(
                name=name,
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_BR6D_PBR:
        used_cfg = SPLITS_BR6D_PBR[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, br6d_Dataset(used_cfg))
...
def get_available_datasets():
    return list(SPLITS_BR6D_PBR.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m this_module br6d_pbr_train
    """
