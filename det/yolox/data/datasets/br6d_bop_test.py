...
class br6d_BOP_TEST_Dataset:
    ...
    def __init__(self, data_cfg):
        ...
        self.dataset_root = data_cfg["dataset_root"]  # BOP_DATASETS/br6d/test
        self.models_root = data_cfg["models_root"]  # BOP_DATASETS/br6d/models
        ...
        self.cat_ids = [cat_id for cat_id, obj_name in ref.br6d.id2obj.items() if obj_name in self.objs]
    ...
    def models(self):
        ...
        for obj_name in self.objs:
            model = inout.load_ply(
                osp.join(self.models_root, f"obj_{ref.br6d.obj2id[obj_name]:06d}.ply"),
...
def get_br6d_metadata(obj_names, ref_key):
...
br6d_BOP19_20_OBJS = [ref.br6d.id2obj[_i] for _i in [3]]

SPLITS_br6d = dict(
    br6d_test=dict(
        name="br6d_test",
        dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/test"),
        models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/models"),
        # objs=br6d_BOP19_20_OBJS,  # selected 16 objects
        objs=ref.br6d.objects,  # all 33 objects
        ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/test_targets_bop19.json"),
    ...
    # br6d_test_primesense_all=dict(
    #     name="br6d_test_primesense_all",
    #     dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/test_primesense"),
    #     models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/models"),
    #     objs=ref.br6d.objects,  # all 33 objects
    #     ann_file=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/test_targets_all.json"),
    #     scale_to_meter=0.001,
    #     with_masks=True,  # (load masks but may not use it)
    #     with_depth=True,  # (load depth path here, but may not use it)
    #     height=480,
...
for obj in ref.br6d.objects:
    name = "br6d_bop_{}_test".format(obj)
    select_objs = [obj]
    if name not in SPLITS_br6d:
        SPLITS_br6d[name] = dict(
            name=name,
            dataset_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/test"),
            models_root=osp.join(DATASETS_ROOT, "BOP_DATASETS/br6d/models"),
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_br6d:
        used_cfg = SPLITS_br6d[name]
    else:
        assert data_cfg is not None, f"dataset name {name} is not registered"
        used_cfg = data_cfg
    DatasetCatalog.register(name, br6d_BOP_TEST_Dataset(used_cfg))
    # something like eval_types
    MetadataCatalog.get(name).set(
        id="br6d",  # NOTE: for pvnet to determine module
        ref_key=used_cfg["ref_key"],
        objs=used_cfg["objs"],
        eval_error_types=["ad", "rete", "proj"],
        evaluator_type="bop",
        **get_br6d_metadata(obj_names=used_cfg["objs"], ref_key=used_cfg["ref_key"]),
    )


def get_available_datasets():
    return list(SPLITS_br6d.keys())
...
if __name__ == "__main__":
    ...
    Usage:
        python -m core.gdrn_modeling.datasets.br6d_bop_test dataset_name
    """
